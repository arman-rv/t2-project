...
const Menu = () => {
  ...
  return (
    <div
      id="menu"
      className="bg-white w-[30%] h-full fixed top-0 z-[1000] hidden max-[1020px]:block transition-opacity duration-500 rounded-l-[15px] border max-[900px]:w-[40%] max-[800px]:w-[35%] shadow-[0_0_70px_#777] max-[700px]:w-[43%] max-[600px]:w-[53%] max-[450px]:w-[100%] max-[450px]:rounded-none overflow-y-auto"
    ...
    >
      <div className=" w-full h-[50px] flex items-center pl-[15px]">
        <img
          ...
          alt=""
          className=" w-[25px] h-[25px] cursor-pointer [&:hover]:drop-shadow-[0_0_1px_#5A0BA9]"
          id="closePic"
        />
        <img
          src="/images/logo.png"
          className="w-full h-full scale-75 max-[600px]:scale-[60%]"
        />
      </div>
      {!token && (
        <>
          <div className="w-full h-[80px] flex [&>div>a]:text-[13px] [&>div>a]:text-[#777] [&>div>img]:ml-[10px]">
            <Link
              to={"/login"}
              className="hover:bg-[#eee] h-[50%] my-auto w-[50%] flex justify-center items-center max-[600px]:[&>div]:text-[13px]"
      ...
      >
        <Linkes
          ...
          content="خانه"
          acceptedClassName="bg-gradient-to-r from-[#9f009a] to-[#fff] text-[#000]"
          ejectedClassName="bg-none text-[#888]"
        />
        <Linkes
          to={"/courses"}

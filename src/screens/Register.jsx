...
const Register = () => {
  ...
  const resendCode = async () => {
    await customAxios.post("/Sign/SendVerifyMessage", {
  ...
  const registerSubmit = async (values) => {
    if (flag == 1) {
      let result = await customAxios.post("/Sign/SendVerifyMessage", {
    ...
    <>
      ...
      <div className="w-full h-[1200px] bg-[#f5f5f5] dark:bg-[#182239]">
        ...
        <div className="w-[1920px] max-[1919px]:w-full  mx-auto h-full relative overflow-hidden flex flex-col justify-start items-center">
          <img
            src="/images/panel/path register.png"
            alt=""
            className="absolute left-[-31px] top-0 scale-x-[122%] scale-y-[127%] max-[1000px]:hidden"
          />
          <img
            src="/images/panel/register logo.png"
            alt=""
            className="absolute left-0 top-[260px] w-[63%] transition-all duration-1000 max-[1485px]:top-[200px] order-2 max-[1256px]:mt-20 max-[1256px]:right-0 max-[1256px]:top-0 max-[1256px]:relative max-[700px]:hidden"
          />
          ...
          <Formik
            ...
            onSubmit={(values) => registerSubmit(values, setFlag, flag)}
            validationSchema={Validation}
          >
            <Form
              className={
                flag !== 3 ? formClassName : formClassName + "h-[550px]"
              }
            >
              <img
              ...
              {flag == 1 && (
                <FieldInput
                  ...
                  dir="rtl"
                  border="border border-[#a361a1]"
                  display="text-[#999] text-[15px]"
              ...
              {flag == 2 && (
                <div className="flex justify-between w-[85%]">
                  <span
                    className="mt-[-10px] text-[15px] self-end cursor-pointer border-b-1 border-b border-b-black hover:text-[#8b61a3] hover:border-b-[#8b61a3]"
                    onClick={editPhoneNumber}
                  >
                    ویرایش شماره همراه
                  </span>
                  <span className="self-start" ref={sendAgainIn}>
                    ارسال مجدد کد در : {codeM}:{codeS}{" "}
                  </span>
                  <span
                    className="self-start hidden cursor-pointer"
                    ref={sendAgain}
                    onClick={resendCode}
                  >
                    {" "}
                    <span className="text-[#bbb] cursor-default">
                      {" "}
                      کدی دریافت نکردید؟{" "}
                    </span>{" "}
                    ارسال مجدد کد{" "}
              ...
              <LinkComponent
                ...
                link="/login"
                className="text-[#564FCC] text-[20px]"
              />
            </Form>
          </Formik>
        </div>
